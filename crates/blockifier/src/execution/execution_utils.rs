...
use crate::execution::entry_point::{
    ...
    EntryPointExecutionResult, ExecutableCallEntryPoint, execute_constructor_entry_point,
};
...
pub fn execute_entry_point_call_wrapper(
    ...
    log::debug!(">>>>> the tracked resources are: {:?}", current_tracked_resource);
    ...
    match res {
        Ok(call_info) => {
            ...
            println!("[APOORV] gas after running execute tx is: {:?}", remaining_gas);
            Ok(call_info)
        }
